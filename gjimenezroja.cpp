   ...
   unsigned char *data2 = new unsigned char 
    ...
    for (int i=0; i<enemy.height; i++) {
        for (int j=0; j<enemy.width; j++) {
            ...
            data2[offset2+0] = enemy.data[offset+0];
            data2[offset2+1] = enemy.data[offset+1];
            data2[offset2+2] = enemy.data[offset+2];
            data2[offset2+3] =
    ...
    //sprite link
    glGenTextures(1, &s.spriteid_two);
    glBindTexture(GL_TEXTURE_2D, s.spriteid_two);
    glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MAG_FILTER,GL_NEAREST);
    glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MIN_FILTER,GL_NEAREST);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, enemy.width, enemy.height, 0,
                                   GL_RGBA, GL_UNSIGNED_BYTE, data2);
    delete [] data2;
    s.sprite_two[0].set_dimensions(s.xres, s.yres);
}
...
void physics_level_two() {
    ...
    if (s.frameno > 20)
        s.frameno = 1;
    //movement
    s.sprite_two[0].pos[0] += s.sprite_two[0].vel[0];
    s.sprite_two[0].pos[1] += s.sprite_two[0].vel[1];
    //boundry test
    if (s.sprite_two[0].pos[0] >= s.xres) {
        s.sprite_two[0].pos[0] = s.xres;
        s.sprite_two[0].vel[0] = 0.0;
    }
    if (s.sprite_two[0].pos[0] <= 0) {
        s.sprite_two[0].pos[0] = 0;
        s.sprite_two[0].vel[0] = 0.0;
    }
    if (s.sprite_two[0].pos[1] >= s.yres) {
        s.sprite_two[0].pos[1] = s.yres;
        s.sprite_two[0].vel[1] = 0.0;
    }
    if (s.sprite_two[0].pos[1] <= 0) {
        s.sprite_two[0].pos[1] = 0;
        s.sprite_two[0].vel[1] = 0.0;
    }
    ...
    Flt dx = cx - s.sprite_two[0].pos[0];
    Flt dy = cy - s.sprite_two[0].pos[1];
    Flt dist = (dx*dx + dy*dy);
    if (dist < 0.01)
        dist = 0.01; //clamp
    s.sprite_two[0].vel[0] += (dx / dist) * s.gravity;
    s.sprite_two[0].vel[1] += (dy / dist) * s.gravity;
    s.sprite_two[0].vel[0] += ((Flt)rand() / (Flt)RAND_MAX) * 0.5 - 0.25;
    s.sprite_two[0].vel[1] += ((Flt)rand() / (Flt)RAND_MAX) * 0.5 - 0.25;
}
void render_level_three() {
    if (s.state == STATE_LEVEL_THREE) {
      ...
      //Draw hylian sprite
        glPushMatrix();
        glColor3ub(255, 255, 255);
        glTranslatef(s.sprite_two[0].pos[0], s.sprite_two[0].pos[1], 0.0f);
        //set alpha test
        ...
        glBindTexture(GL_TEXTURE_2D, s.spriteid_two);
        glBegin(GL_QUADS);
            glTexCoord2f(0, 1); glVertex2f(-s.sprite_two[0].w, 
                                          -s.sprite_two[0].h);
            glTexCoord2f(0, 0); glVertex2f(-s.sprite_two[0].w,  
                                           s.sprite_two[0].h);
            glTexCoord2f(1, 0); glVertex2f( s.sprite_two[0].w,  
                                           s.sprite_two[0].h);
            glTexCoord2f(1, 1); glVertex2f( s.sprite_two[0].w, 
                                          -s.sprite_two[0].h);
        glEnd();
        glBindTexture(GL_TEXTURE_2D, 0);
